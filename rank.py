...
class Crawl(object):
    def get_href(self, start):
        url = 'https://search.naver.com/search.naver?where=news&sm=tab_jum&query=%EC%98%A4%EB%8A%98&sort=0&photo=0&field=0&pd=0&ds=&de=&cluster_rank=0&mynews=0&office_type=0&office_section_code=0&news_office_checked=&nso=so:r,p:all,a:all&start=' + str(start)
...
def insert_news(keyword, summary, url):
    conn = pymysql.connect(host='172.18.0.25', user='root', password='news', db='news', charset='utf8')
    try:
        with conn.cursor() as curs:
            sql = 'insert into news(keyword, summary, url) values(%s, %s, %s)'
...
for url in url_list:
    try:
        ...
        summary = '. '.join(map(str, textrank.summarize(3))).replace('\n\n', '') + '.'
...
#url = 'https://www.docdocdoc.co.kr/news/articleView.html?idxno=2006596'
#try:
#    textrank = TextRank(url)
#    keyword = ' '.join(map(str, textrank.keywords(3)))
#    summary = '. '.join(map(str, textrank.summarize(3))).replace('\n\n', '') + '.'
...
#s= SentenceTokenizer()
#senstences = s.text2sentences(string)
